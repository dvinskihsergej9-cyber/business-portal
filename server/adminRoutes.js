...
export function adminRoutes({ prisma, auth, requireAdmin }) {
  ...
  router.post("/warehouse/items/reset-single", async (req, res) => {
    try {
      ...
      return res.json({
        message: "Номенклатура сброшена до одного товара.",
        item: result,
      });
    } catch (err) {
      console.error("admin items reset error:", err);
      return res.status(500).json({ message: "Ошибка сброса номенклатуры" });
    }
  });
