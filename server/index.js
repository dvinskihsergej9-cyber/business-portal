...
app.use("/api/crossdock", auth, enforceOperationalTenantScope, requirePermission(PERMISSION_KEYS.APP_CROSSDOCK));
...
const CROSSDOCK_DIRECTIONS = new Set(["INBOUND", "OUTBOUND"]);
const CROSSDOCK_STATUSES = new Set([
  "PLANNED",
  "ARRIVED",
  "AT_DOCK",
  "IN_PROGRESS",
...
const CROSSDOCK_FLOW = ["PLANNED", "ARRIVED", "AT_DOCK", "IN_PROGRESS", "DONE"];

function normalizeCrossdockDirection(value) {
  const normalized = String(value || "").trim().toUpperCase();
  return CROSSDOCK_DIRECTIONS.has(normalized) ? normalized : null;
}

function normalizeCrossdockStatus(value, fallback = null) {
  const normalized = String(value || "").trim().toUpperCase();
  if (CROSSDOCK_STATUSES.has(normalized)) return normalized;
  return fallback;
}

function normalizeCrossdockText(value, maxLen = 255) {
  const normalized = String(value || "").trim();
  if (!normalized) return null;
  return normalized.slice(0, maxLen);
}

function parseCrossdockDateTime(value) {
  if (!value) return null;
...
function canMoveCrossdockStatus(fromStatus, toStatus) {
  if (!fromStatus || !toStatus) return false;
  if (fromStatus === toStatus) return true;
  if (fromStatus === "DONE") return false;
  if (fromStatus === "CANCELLED") return false;
  if (toStatus === "CANCELLED") return true;
  const fromIndex = CROSSDOCK_FLOW.indexOf(fromStatus);
  const toIndex = CROSSDOCK_FLOW.indexOf(toStatus);
  if (fromIndex < 0 || toIndex < 0) return false;
  return toIndex >= fromIndex;
}

function buildCrossdockStatusTimestamps(current, nextStatus, now = new Date()) {
  const data = {};
  if (nextStatus === "ARRIVED") {
    data.actualArrivalAt = current.actualArrivalAt || now;
  }
  if (nextStatus === "AT_DOCK") {
    data.actualArrivalAt = current.actualArrivalAt || now;
    data.atDockAt = current.atDockAt || now;
  }
  ...
  if (nextStatus === "CANCELLED") {
    data.cancelledAt = current.cancelledAt || now;
  }
  return data;
}

app.get("/api/crossdock/docks", auth, async (req, res) => {
  try {
    ...
    const docks = await prisma.crossdockDock.findMany({
      where,
      orderBy: [{ isActive: "desc" }, { code: "asc" }],
      take: 300,
    });

    res.json({ items: docks });
  } catch (err) {
    console.error("crossdock docks list error:", err);
    res.status(500).json({ message: "CROSSDOCK_DOCKS_LIST_ERROR" });
  }
});

app.post("/api/crossdock/docks", auth, async (req, res) => {
  try {
    if (!isWarehouseManager(req.user)) {
      return res.status(403).json({ message: "CROSSDOCK_DOCK_FORBIDDEN" });
    }

    const code = normalizeCrossdockText(req.body?.code, 40)?.toUpperCase();
    const name = normalizeCrossdockText(req.body?.name, 120);
    const note = normalizeCrossdockText(req.body?.note, 500);

    if (!code) {
      return res.status(400).json({ message: "CROSSDOCK_DOCK_CODE_REQUIRED" });
    }

    const dock = await prisma.crossdockDock.create({
      data: {
        code,
        name,
        note,
    ...
    return res.status(201).json({ item: dock });
  } catch (err) {
    if (err?.code === "P2002") {
      return res.status(409).json({ message: "CROSSDOCK_DOCK_CODE_EXISTS" });
    }
    console.error("crossdock dock create error:", err);
    return res.status(500).json({ message: "CROSSDOCK_DOCK_CREATE_ERROR" });
  }
});

app.patch("/api/crossdock/docks/:id", auth, async (req, res) => {
  try {
    if (!isWarehouseManager(req.user)) {
      return res.status(403).json({ message: "CROSSDOCK_DOCK_FORBIDDEN" });
    }
    ...
    const id = Number(req.params.id);
    if (!id || Number.isNaN(id)) {
      return res.status(400).json({ message: "CROSSDOCK_DOCK_BAD_ID" });
    }

    const current = await prisma.crossdockDock.findUnique({ where: { id } });
    if (!current) {
      return res.status(404).json({ message: "CROSSDOCK_DOCK_NOT_FOUND" });
    }

    const nextCodeRaw = normalizeCrossdockText(req.body?.code, 40);
    const nextCode = nextCodeRaw ? nextCodeRaw.toUpperCase() : null;
    const patch = {};

    if (nextCode) patch.code = nextCode;
    if (req.body?.name !== undefined) patch.name = normalizeCrossdockText(req.body?.name, 120);
    if (req.body?.note !== undefined) patch.note = normalizeCrossdockText(req.body?.note, 500);
    if (req.body?.isActive !== undefined) patch.isActive = Boolean(req.body.isActive);

    const updated = await prisma.crossdockDock.update({
      where: { id },
      data: patch,
    });
    ...
    console.error("crossdock dock update error:", err);
    return res.status(500).json({ message: "CROSSDOCK_DOCK_UPDATE_ERROR" });
  }
});

app.get("/api/crossdock/operations", auth, async (req, res) => {
  try {
    const q = String(req.query.q || "").trim();
    const direction = normalizeCrossdockDirection(req.query.direction);
    const status = normalizeCrossdockStatus(req.query.status);
    const dockId = Number(req.query.dockId || 0);
    const dateFrom = parseCrossdockDateTime(req.query.dateFrom);
    const dateTo = parseCrossdockDateTime(req.query.dateTo);
    const page = Math.max(1, Number(req.query.page) || 1);
    const limit = Math.min(200, Math.max(1, Number(req.query.limit) || 50));
    const skip = (page - 1) * limit;
    ...
    if (dockId > 0) where.dockId = dockId;
    ...
    if (q) {
      where.OR = [
        ...
        { cargoSummary: { contains: q, mode: "insensitive" } },
      ];
    }

    const [total, items] = await Promise.all([
      prisma.crossdockOperation.count({ where }),
      prisma.crossdockOperation.findMany({
        where,
        orderBy: [{ createdAt: "desc" }, { id: "desc" }],
        skip,
        take: limit,
    ...
    console.error("crossdock operations list error:", err);
    return res.status(500).json({ message: "CROSSDOCK_OPERATIONS_LIST_ERROR" });
  }
});

app.get("/api/crossdock/operations/:id", auth, async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!id || Number.isNaN(id)) {
      return res.status(400).json({ message: "CROSSDOCK_OPERATION_BAD_ID" });
    }

    const item = await prisma.crossdockOperation.findUnique({
      where: { id },
      include: {
        dock: true,
        createdBy: { select: { id: true, name: true, email: true } },
    ...
    if (!item) {
      return res.status(404).json({ message: "CROSSDOCK_OPERATION_NOT_FOUND" });
    }

    return res.json({ item });
  } catch (err) {
    console.error("crossdock operation get error:", err);
    return res.status(500).json({ message: "CROSSDOCK_OPERATION_GET_ERROR" });
  }
});

app.post("/api/crossdock/operations", auth, async (req, res) => {
  try {
    if (!isWarehouseManager(req.user)) {
      return res.status(403).json({ message: "CROSSDOCK_OPERATION_FORBIDDEN" });
    }

    const direction = normalizeCrossdockDirection(req.body?.direction);
    if (!direction) {
      return res.status(400).json({ message: "CROSSDOCK_DIRECTION_REQUIRED" });
    }

    const status = normalizeCrossdockStatus(req.body?.status, "PLANNED");
    const dockId = Number(req.body?.dockId || 0) || null;

    if (dockId) {
      const dock = await prisma.crossdockDock.findUnique({ where: { id: dockId } });
      if (!dock) {
        return res.status(404).json({ message: "CROSSDOCK_DOCK_NOT_FOUND" });
      }
    }
    ...
    const item = await prisma.crossdockOperation.create({
      data: {
        direction,
        status,
        dockId,
        referenceNumber: normalizeCrossdockText(req.body?.referenceNumber, 80),
        partnerName: normalizeCrossdockText(req.body?.partnerName, 160),
        truckNumber: normalizeCrossdockText(req.body?.truckNumber, 80),
        cargoSummary: normalizeCrossdockText(req.body?.cargoSummary, 500),
        plannedAt: parseCrossdockDateTime(req.body?.plannedAt),
        note: normalizeCrossdockText(req.body?.note, 1000),
        createdByUserId: req.user?.id || null,
        updatedByUserId: req.user?.id || null,
        ...buildCrossdockStatusTimestamps(draftForTimestamps, status, now),
      },
      include: {
        dock: true,
        createdBy: { select: { id: true, name: true, email: true } },
    ...
    console.error("crossdock operation create error:", err);
    return res.status(500).json({ message: "CROSSDOCK_OPERATION_CREATE_ERROR" });
  }
});

app.patch("/api/crossdock/operations/:id", auth, async (req, res) => {
  try {
    if (!isWarehouseManager(req.user)) {
      return res.status(403).json({ message: "CROSSDOCK_OPERATION_FORBIDDEN" });
    }
    ...
    const current = await prisma.crossdockOperation.findUnique({ where: { id } });
    if (!current) {
      return res.status(404).json({ message: "CROSSDOCK_OPERATION_NOT_FOUND" });
    }
    ...
    if (req.body?.direction !== undefined) {
      const direction = normalizeCrossdockDirection(req.body?.direction);
      if (!direction) {
        return res.status(400).json({ message: "CROSSDOCK_DIRECTION_INVALID" });
      }
      patch.direction = direction;
    }

    if (req.body?.status !== undefined) {
      const nextStatus = normalizeCrossdockStatus(req.body?.status);
      if (!nextStatus) {
        return res.status(400).json({ message: "CROSSDOCK_STATUS_INVALID" });
      }
      if (!canMoveCrossdockStatus(current.status, nextStatus)) {
        return res.status(409).json({ message: "CROSSDOCK_STATUS_TRANSITION_DENIED" });
      }
      patch.status = nextStatus;
      Object.assign(patch, buildCrossdockStatusTimestamps(current, nextStatus, new Date()));
    }

    if (req.body?.dockId !== undefined) {
      const dockId = Number(req.body?.dockId || 0) || null;
      if (dockId) {
        const dock = await prisma.crossdockDock.findUnique({ where: { id: dockId } });
        if (!dock) {
    ...
    if (req.body?.referenceNumber !== undefined) {
      patch.referenceNumber = normalizeCrossdockText(req.body?.referenceNumber, 80);
    }
    if (req.body?.partnerName !== undefined) {
      patch.partnerName = normalizeCrossdockText(req.body?.partnerName, 160);
    }
    if (req.body?.truckNumber !== undefined) {
      patch.truckNumber = normalizeCrossdockText(req.body?.truckNumber, 80);
    }
    if (req.body?.cargoSummary !== undefined) {
      patch.cargoSummary = normalizeCrossdockText(req.body?.cargoSummary, 500);
    }
    if (req.body?.plannedAt !== undefined) {
      patch.plannedAt = parseCrossdockDateTime(req.body?.plannedAt);
    }
    if (req.body?.note !== undefined) {
      patch.note = normalizeCrossdockText(req.body?.note, 1000);
    }

    const item = await prisma.crossdockOperation.update({
      where: { id },
      data: patch,
      include: {
        dock: true,
    ...
    console.error("crossdock operation update error:", err);
    return res.status(500).json({ message: "CROSSDOCK_OPERATION_UPDATE_ERROR" });
  }
});

app.post("/api/crossdock/operations/:id/status", auth, async (req, res) => {
  try {
    if (!isWarehouseManager(req.user)) {
      return res.status(403).json({ message: "CROSSDOCK_OPERATION_FORBIDDEN" });
    }
    ...
    const patch = {
      status: nextStatus,
      updatedByUserId: req.user?.id || null,
      ...buildCrossdockStatusTimestamps(current, nextStatus, new Date()),
    };
    ...
    console.error("crossdock operation status update error:", err);
    return res.status(500).json({ message: "CROSSDOCK_OPERATION_STATUS_ERROR" });
  }
});
