# ����������� (архивная база + восстановление мобильного и оплаты)
...
## Order shipment by passport QR
- Passport PDF now contains QR payload in format: `bp:order:<ORDER_ID>`.
- In Mobile TSD, use mode `????????`:
  - scan passport QR (or enter `bp:order:<id>` / order id manually),
  - optional fields: `carrier`, `trackingNumber`, `notes`,
  - both actions `???????????` and `??????????` call `/api/orders/:id/ship`.
- Shipment API changes:
  - `POST /api/orders/:id/ship` moves order `READY_TO_SHIP -> SHIPPED`,
  - stores `shippedAt`, `shippedByUserId`, and optional shipment fields.
- Status history:
  - `GET /api/orders/status-history` (global journal with filters),
  - `GET /api/orders/:id/status-history` (order timeline),
  - `/api/orders/:id/complete` writes `COMPLETE` event,
  - `/api/orders/:id/ship` writes `SHIP` event.
